...
function setView(viewId) {
  ...
  if (viewId === "phaseTwo") renderPhaseModule("result");
}
...
function moduleRows(moduleId) {
  const students = getStudents();
  const rows = {
    result: students.map((student) => [student.id, student.name, student.cgpa || "N/A", student.results || "No result added"]),
    payment: students.map((student) => [student.id, student.name, student.payment || "Due", student.email || ""]),
    registration: students.map((student) => [student.id, student.name, student.program || "N/A", `Batch ${student.batch || "-"}`]),
    attendance: students.map((student) => [student.id, student.name, "Not added", "Ready for Phase 2"]),
    certificate: students.map((student) => [student.id, student.name, "No request", "Ready for Phase 2"])
  };
  return rows[moduleId] || [];
}

function renderPhaseModule(moduleId) {
  const panel = $("#phaseModulePanel");
  if (!panel) return;

  document.querySelectorAll("[data-phase]").forEach((card) => {
    card.classList.toggle("active", card.dataset.phase === moduleId);
  });
  ...
  if (moduleId === "notice") {
    ...
    panel.innerHTML = `
      <div class="panel-heading">
        <div>
          <h2>Notice</h2>
          <p>Published notices from the admin panel.</p>
        </div>
        <button class="ghost-button" type="button" data-view="adminDashboard">Publish Notice</button>
      </div>
      <div class="phase-list">
        ...
        `).join("") || `<div class="phase-empty">No notice published yet.</div>`}
      </div>
    `;
    return;
  }
  ...
  const config = {
    result: ["Result", ["Student ID", "Name", "CGPA", "Result"]],
    payment: ["Payment", ["Student ID", "Name", "Status", "Email"]],
    registration: ["Course Registration", ["Student ID", "Name", "Program", "Batch"]],
    attendance: ["Attendance", ["Student ID", "Name", "Attendance", "Status"]],
    certificate: ["Certificate Request", ["Student ID", "Name", "Request", "Status"]]
  }[moduleId];
  const rows = moduleRows(moduleId);

  panel.innerHTML = `
    <div class="panel-heading">
      <div>
        <h2>${config[0]}</h2>
        <p>${rows.length} student records loaded.</p>
      </div>
      <button class="ghost-button" type="button" data-view="studentDirectory">Manage Students</button>
    </div>
    <div class="table-wrap compact-table">
      <table>
        <thead>
          <tr>${config[1].map((heading) => `<th>${heading}</th>`).join("")}</tr>
        </thead>
        <tbody>
          ${rows.map((row) => `<tr>${row.map((cell) => `<td>${cell}</td>`).join("")}</tr>`).join("") || `<tr><td colspan="${config[1].length}">No student data found.</td></tr>`}
        </tbody>
      </table>
    </div>
  `;
...
function fillForm(student) {
  ...
  $("#passingYear").value = student.passingYear || "";
...
document.addEventListener("click", (event) => {
  ...
  if (phaseCard) renderPhaseModule(phaseCard.dataset.phase);
...
document.addEventListener("keydown", (event) => {
  ...
  if (event.key === "Enter" || event.key === " ") {
    event.preventDefault();
    renderPhaseModule(phaseCard.dataset.phase);
  }
});
